...
- 角度相关：单位为 **度（°）**。源码中大量使用 `* 57.29578f`（即 rad→deg）。
- 角速度相关：单位为 **度每秒（°/s）**。来源是 `angularVelocity`（rad/s）再乘 `57.29578f`。
- 时间相关：单位为 **秒（s）**（如 `Time.time`、`deltaTime`）。
- 距离/高度/坐标相关：使用游戏世界坐标单位（Unity world unit，工程中通常按米理解）。
- 速度相关：世界坐标单位每秒（通常按 **m/s** 理解）。
- `Latitude`/`Longitude` 在该工程里是坐标轴值（`z/x`），**不是地理经纬度“度数”**。
